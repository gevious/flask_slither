...
class NamespaceTests(TestCase):
    """ Test urls with a global namespace, or mount point.

        For example if the app has a namespace of `foo`, we want to set up
        `/bar` without knowing about foo. The namespace is independent of
        registered urls."""
    def create_app(self):
        app = Flask(__name__)
        app.url_map.converters['regex'] = RegexConverter
    ...
    def test_get_root(self):
        register_api(self.app, Resource)
        with self.app.test_client() as c:
            response = c.get('/')
            self.assertEquals(response.status_code, 200)

    def test_get_path(self):
        register_api(self.app, Resource, url="bar")
        with self.app.test_client() as c:
            response = c.get('/bar')
            self.assertEquals(response.status_code, 200)

    def test_get_mounted_root(self):
        self.app.namespace = 'foo'
        register_api(self.app, Resource)
        with self.app.test_client() as c:
            response = c.get('/foo')
            self.assertEquals(response.status_code, 200)

    def test_get_mounted_root_leading(self):
        self.app.namespace = '/foo'
        register_api(self.app, Resource)
        with self.app.test_client() as c:
            response = c.get('/foo')
    ...
    def test_get_mounted_root_trailing(self):
        self.app.namespace = 'foo/'
        register_api(self.app, Resource)
        with self.app.test_client() as c:
            response = c.get('/foo')
    ...
    def test_get_mounted_path(self):
        self.app.namespace = 'foo'
        register_api(self.app, Resource, url='bar')
        with self.app.test_client() as c:
    ...
    def test_get_mounted_path_trailing(self):
        self.app.namespace = 'foo'
        register_api(self.app, Resource, url='bar/')
        with self.app.test_client() as c:
            response = c.get('/foo/bar')
    ...
    def test_get_mounted_path_leading(self):
        self.app.namespace = 'foo'
        register_api(self.app, Resource, url='/bar')
        with self.app.test_client() as c:
            response = c.get('/foo/bar')
